...
from backend.app.schemas import SummaryStatsAnalysis, Filter, ReportBlock, ColumnTransformation, Transformation
...
def test_summary_stats_no_grouping(sample_df):
    ...
    sales_mean = results_df.loc[
        (results_df['Column'] == 'sales') & (results_df['Metric'] == 'mean'),
        'Value'
    ].iloc[0]
    ...
    units_count = results_df.loc[
        (results_df['Column'] == 'units') & (results_df['Metric'] == 'count'),
        'Value'
    ].iloc[0]
...
def test_summary_stats_single_group(sample_df):
    ...
    na_sales_mean = results_df.loc[
        (results_df['Group'] == 'NA') & (results_df['Metric'] == 'mean'),
        'Value'
    ].iloc[0]
    ...
    eu_sales_mean = results_df.loc[
        (results_df['Group'] == 'EU') & (results_df['Metric'] == 'mean'),
        'Value'
    ].iloc[0]
...
def test_summary_stats_multi_group(sample_df):
    ...
    na_a_mean = results_df.loc[
        (results_df['Group'] == 'NA, A') & (results_df['Metric'] == 'mean'),
        'Value'
    ].iloc[0]
...
def test_summary_stats_with_transformation(sample_df):
    ...
    units_mean = results_df.loc[
        (results_df['Column'] == 'units') & (results_df['Metric'] == 'mean'),
        'Value'
    ].iloc[0]
...
def test_summary_stats_filter_yields_no_rows(sample_df):
    ...
    step = SummaryStatsAnalysis(
        ...
        filters=[Filter(column="region", operator="eq", value="ASIA")], # No ASIA region
        numeric_columns=["sales"]
    )
    result = run(sample_df, step)
