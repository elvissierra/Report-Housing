...
def test_outlier_iqr_finds_extreme_values(sample_df):
    ...
    step = OutlierDetectionAnalysis(
        target_columns=["sales"], method="iqr", threshold=1.5, output_name="OD Test"
    )
    ...
    assert (
        0
        in results_df[results_df["Outlier Value"] == -3000]["Original Row Index"].values
    )
    assert (
        9
        in results_df[results_df["Outlier Value"] == 5000]["Original Row Index"].values
    )
...
def test_outlier_zscore_finds_extreme_values(sample_df):
    ...
    step = OutlierDetectionAnalysis(
        target_columns=["sales"], method="z-score", threshold=1.5, output_name="OD Test"
    )
    result = run(sample_df, step)
...
def test_outlier_iqr_no_outliers_found(sample_df):
    ...
    step = OutlierDetectionAnalysis(
        target_columns=["units"], method="iqr", output_name="OD Test"
    )
    result = run(sample_df, step)
...
def test_outlier_threshold_parameter(sample_df):
    ...
    step = OutlierDetectionAnalysis(
        target_columns=["sales"], method="iqr", threshold=6.0, output_name="OD Test"
    )
